...
Progress is tracked in [`Product roadmap and progress.md`](Product%20roadmap%20and%20progress.md). The current estimate is **44% overall**, weighted toward the working Cameroon discovery foundation, the Local/Remote and PWA foundation, live global feed adapters, the visible section navigation, and the first in-site editable ApplyFlow queue. Account persistence, private document storage, direct submission adapters, Simplify import, swipe interactions, deeper eligibility, and controlled automation are mostly planned rather than implemented.
...
| ApplyFlow | Swipe queue, Application Pack, tracker | User can move from match to reviewed application and follow-up status. The current milestone is a 22% in-site editable pack and local queue with explicit approval, recovery states, manual fallback, and no silent submission. |
