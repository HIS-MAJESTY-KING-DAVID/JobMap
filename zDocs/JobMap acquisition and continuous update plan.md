...
Progress is tracked in [`Product roadmap and progress.md`](Product%20roadmap%20and%20progress.md). The current estimate is **41% overall**, weighted toward the working Cameroon discovery foundation, the Local/Remote and PWA foundation, live global feed adapters, the visible section navigation, and the first profile-aware ApplyFlow panel. Account persistence, private document storage, Simplify import, swipe interactions, deeper eligibility, and controlled automation are mostly planned rather than implemented.
...
| ApplyFlow | Swipe queue, Application Pack, tracker | User can move from match to reviewed application and follow-up status. The current milestone is a 14% profile-aware guided review surface with canonical handoff and no silent submission. |
